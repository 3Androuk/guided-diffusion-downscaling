...
def res_scale(res_std, like):
    """Residual scale, broadcastable over (N, C, H, W).

    Legacy checkpoints store res_std as ONE float for every channel -- exact for
    a single-variable model, wrong for 20 (see train_residual --per-channel-res-std).
    New ones store a per-channel sequence. Accepts either, so old checkpoints
    keep reproducing bit-for-bit.
    """
    import torch as _t
    if isinstance(res_std, (int, float)):
        return float(res_std)
    v = _t.as_tensor(res_std, dtype=like.dtype, device=like.device)
    return v.view(1, -1, *([1] * (like.dim() - 2)))
