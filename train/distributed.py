...
def barrier(ctx: DistContext) -> None:
    """Block until every rank arrives; no-op single-process.

    Used before a rank reads a checkpoint another rank wrote — rank 0 does the
    saving, so without this a faster rank can open a file mid-write.
    """
