...
from train.guards import restore_best, spike_recover, spike_skip  # noqa: E402
...
def run(method: str):
    ...
    loader = make_train_loader(dataset, train_cfg["batch_size"],
                               train_cfg["num_workers"], dist, seed=cfg["seed"])
    ...
    best_ckpt_path = ckpt_path.with_name(f"{stem}_best.pt")
    # Collapse protection (train/guards.py). Four of six 20-var arms collapsed
    # mid-run and this trainer had none; the mean map this arm depends on
    # exploded 80 times in 56 epochs.
    spike = build_spike_guard(train_cfg)
    guard = build_divergence_guard(train_cfg)
    rollbacks_left = int(train_cfg.get("divergence", {}).get("rollbacks", 0))
    reshuffle_offset = 0
    best_val = float("inf")
    ...
    if dist.enabled:
        ...
        # the noise draws, transport times, and per-batch ratio choices.
        set_seed(cfg["seed"] + dist.rank)

    # Rank 0 only: weights are identical on every rank, so one process scoring
    # the full val set reproduces single-process values (and gates sampling).
    ...
    for epoch in range(start_epoch, train_cfg["epochs"] + 1):
        ...
        for batch in loader:
            ...
            if spike_skip(spike, grad_norm, scaler, step, dist.is_main):
                # No optimizer.step() -> Adam's moments never see the bad
                # direction; no ema.update() -> EMA keeps pre-spike weights.
                # scaler.update() is still required or the next unscale_ raises.
                spike_recover(spike, dist.is_main, step)
                scaler.update()
                step += 1
                continue
            ...
            if step % train_cfg["log_every"] == 0:
                ...
                metrics = {
                    ...
                    "train/imgs_per_sec": (running_n * target.shape[0]
                                           * dist.world_size / (now - last_log)),
                    "train/ratio": ratio,
                    "epoch": epoch,
                }
        ...
        # val_loader is rank-0 ONLY (see its definition), so val keys are absent
        # elsewhere. The SAVE is rank-0 file I/O and may be conditional; the
        # BROADCAST must not be — a collective reached by one rank hangs the
        # others, which is how job 6236360 died (NCCL ALLREDUCE timeout).
        # Averaged over every val/* key so the guard does not depend on which
        # ratios _validate happens to report.
        vkeys = [v for k, v in epoch_metrics.items() if k.startswith("val/")]
        val_now = float(np.mean(vkeys)) if vkeys else None
        if dist.is_main and val_now is not None and val_now < best_val:
            best_val = val_now
            _save_checkpoint(best_ckpt_path, raw_model, ema, optimizer, scaler,
                             cfg, normalizer, method, epoch, step,
        ...
        if guard is not None and broadcast_flag(reason is not None, dist):
            if rollbacks_left > 0 and best_ckpt_path.exists():
                ...
                barrier(dist)
                restore_best(best_ckpt_path, raw_model, ema, optimizer, scaler, device)
        ...
        if epoch % train_cfg.get("ckpt_every_epochs", 1) == 0 or epoch == train_cfg["epochs"]:
            ...
            if not _weights_finite(raw_model) or not _weights_finite(ema.shadow):
                raise RuntimeError("non-finite transport weights; checkpoint not overwritten")
            if dist.is_main:
                _save_checkpoint(ckpt_path, raw_model, ema, optimizer, scaler, cfg,
...
def _validation_loader(cfg, patch_dir, normalizer, need_coords, geo_cfg=None):
    ...
    # Spread over the WHOLE test split — range(n) took ~32 consecutive
    # January days (patches are time-ordered at 8 per field).
    v_idx = np.linspace(0, len(ds) - 1, n).astype(int).tolist()
    return DataLoader(Subset(ds, v_idx), batch_size=cfg["train"]["batch_size"],
