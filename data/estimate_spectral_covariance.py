...
def estimate(patches, mean, std, indices, batch_size: int,
...
def main():
    ...
    # Multi-variable aware: the periodogram in `estimate` is already per-channel
    # and SpectralCovarianceProjector takes a DIAGONAL channel covariance
    # (C,H,W//2+1), so a 20-channel artifact needs no cross-channel terms.
    ...
    if patches.ndim != 4:
        raise ValueError(f"expected (N,C,H,W) patches, got {patches.shape}")
    if len(patches) == 0:
    ...
    if mean.size not in (1, patches.shape[1]) or std.size != mean.size:
        raise ValueError(f"norm stats size {mean.size} vs {patches.shape[1]} channels")
    ...
    # broadcast over (N,C,H,W); a scalar pair keeps the single-channel behaviour
    ...
    np.savez(
        ...
        mean=np.asarray(mean).ravel(), std=np.asarray(std).ravel(),
        spectrum=np.array(mode),
