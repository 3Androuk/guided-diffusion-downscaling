...
class GaussianDiffusion(nn.Module):
    ...
    def guided_reconstruct(
        ...
        for k in iterator:
            ...
            for i in reversed(range(1, len(seq))):
                ...
                # ORDER MATTERS: physics correction FIRST, projection LAST. DDNM is
                # x <- (I - A'A)x + A'y; the final DDIM step returns x0_pred
                # unchanged, so whatever runs last has the last word. Run after the
                # projection, a constraint can satisfy balance by moving the OBSERVED
                # block means (measured: z500 error 4.4 -> 50 m2/s2). Ordered this way
                # the physics correction is confined to the null space of A.
                # (Ported from claude/wb2-20var-downscaling, 2026-09-03.)
                ...
                if project:
                    if covariance_projector is None:
                        ...
                        # Weather-DDNM: distribute the range-space correction by the
                        # field's own spatial covariance, C A^T (A C A^T)^-1, instead
                        # of the pixel-space pseudoinverse A^T. Observed block means
                        # are preserved exactly either way; only HOW the correction
                        # is spread over the tile changes. Ported from
                        # codex/flow-stochastic-superres (2026-09-04).
    ...
    def sample_unconditional(self, model: nn.Module, shape, device, n_steps=100, eta=0.0,
        ...
        # `noise` lets a tiled caller crop each tile's start from ONE global field,
        # so overlapping tiles agree where they are blended.
