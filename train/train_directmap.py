...
from train.distributed import (barrier, broadcast_flag,  # noqa: E402
                               cleanup, init_distributed,  # noqa: E402
...
from utils import (add_perf_args, apply_perf_overrides,  # noqa: E402
                   resolve_amp,
                   build_divergence_guard, build_spike_guard,
...
def main():
    ...
    ap.add_argument("--ratios", type=int, nargs="+", default=None,
                    help="override the training ratio(s). With --random-ratio this "
                         "replaces residual.train_ratios (the mean's ratio set); "
                         "without it, the first value replaces directmap.train_ratio. "
                         "Omitted = read the config, so existing runs are unchanged.")
    ap.add_argument("--tag", default="",
                    help="suffix for the checkpoint name, so a variant cannot "
                         "overwrite the arm it is compared against.")
    ...
    dc = cfg["directmap"]
    ratio = args.ratios[0] if args.ratios else dc["train_ratio"]
    train_ratios = (
        (args.ratios or cfg.get("residual", {}).get("train_ratios", [2, 4, 8]))
    ...
    if dist.is_main and test_path.exists():
        ...
        if geo_on:
            # Spread over the WHOLE test split: patches are time-ordered at
            # 8 per field, so range(n_val) was ~32 consecutive January days.
            # Gradients never see val, but the BEST-checkpoint choice and the
            # divergence guard both read it.
            v_idx = np.linspace(0, len(val_ds) - 1, n_val).astype(int)
            items = [val_ds[int(i)] for i in v_idx]
            val_x = torch.stack([it[0] for it in items])
            val_coords = torch.stack([it[1] for it in items])
        else:
            v_idx = np.linspace(0, len(val_ds) - 1, n_val).astype(int)
            val_x = torch.stack([val_ds[int(i)] for i in v_idx])
    ...
    # Collapse protection. Four of six 20-var arms collapsed mid-run; this
    # trainer had none. Config lives under train.spike / train.divergence so
    # every arm shares one setting. See train/guards.py.
    tcg = cfg["train"]
    spike = build_spike_guard(tcg)
    guard = build_divergence_guard(tcg)
    rollbacks_left = int(tcg.get("divergence", {}).get("rollbacks", 0))
    reshuffle_offset = 0
    best_val = float("inf")
    ...
    stem = ("meanmap" if args.random_ratio else "directmap") + (f"_{args.tag}" if args.tag else "")
    ...
    for epoch in range(start_epoch, dc["epochs"] + 1):
        ...
        # without it the retry is deterministic and reproduces the collapse.
        ...
        if val_x is not None:
            ...
            with torch.no_grad():
                for r, key in ((ratio, f"val/mse_{ratio}x"), (8, "val/mse_8x")):
                    ...
                    for i in range(0, len(val_x), dc["batch_size"]):
                        ...
                        yv, cv = _centre_crop(y, c, args.crop)
                        total += loss_fn(fwd_eval(degrade(yv, r), cv), yv).item() * yv.shape[0]
                        n += y.shape[0]
            ...
            if wb_run is not None:
                wb_run.log(metrics, step=step)
            val_now = metrics[f"val/mse_{ratio}x"]

            # Best-by-validation checkpoint, so a late collapse costs nothing.
            # Pure rank-0 file I/O — no collective, so it may live in here.
            if dist.is_main and val_now < best_val:
                best_val = val_now
        ...
        # ── Divergence guard — OUTSIDE the `if val_x is not None` block ─────
        # val_x is built on rank 0 ONLY, so anything collective placed inside
        # that block runs on one rank and hangs the rest. That is exactly how
        # job 6236360 died: NCCL ALLREDUCE timeout, ranks 1-3 stuck 600 s at
        # the same sequence number while rank 0 sat in the broadcast. Every
        # rank must reach broadcast_flag; only rank 0 has a verdict to share.
        ...
        if guard is not None and broadcast_flag(reason is not None, dist):
            if rollbacks_left > 0 and best_ckpt_path.exists():
                ...
                if dist.is_main:
                    print(f"DIVERGED at epoch {epoch}: {reason}\n"
                          f"  rolling back to {best_ckpt_path} "
                          f"(val {best_val:.5f}) and continuing with a "
                          f"reshuffled order; {rollbacks_left} left.",
                          flush=True)
                barrier(dist)
                restore_best(best_ckpt_path, raw_model, None, opt, scaler, device)
            ...
            if dist.is_main:
                print(f"DIVERGED at epoch {epoch}: {reason}\nStopping early. "
                      f"The rolling checkpoint holds the diverged weights; the "
...
def _save_recons(fwd, val_batch, val_coords, normalizer, device, ratio, path, model,
    ...
    for i in range(len(y)):
        ...
        yi, ci = _centre_crop(yi, ci, args.crop)
        x4 = degrade(yi, ratio)
        panels.append([
