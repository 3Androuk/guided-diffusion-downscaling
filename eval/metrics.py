...
def spectrum_log_l1_per_channel(pred, truth) -> list:
    """spectrum_log_l1 computed separately for each channel.

    Unlike the physical-unit L2, the pooled spectrum_log_l1 is already fair
    across channels — the log ratio cancels any per-channel scale factor, so a
    variable with 10^6 times the magnitude does not dominate. Splitting it by
    channel is therefore about DIAGNOSIS rather than correctness: it says which
    variables the model reproduces at the right scale-by-scale power and which
    it over- or under-sharpens, which the pooled mean hides.
    """
    p, t = _as_nchw(pred), _as_nchw(truth)
    return [spectrum_log_l1(p[:, c], t[:, c]) for c in range(p.shape[1])]
...
def crps_per_channel(members, truth) -> list:
    """Fair ensemble CRPS computed separately per channel.

    crps_ensemble pools channels (via _to_numpy, which folds C into the sample
    axis) and reports physical units, so the pooled number is dominated by
    whichever variable has the largest magnitude -- exactly the failure
    l2_per_channel exists to avoid. In the 20-var config that means msl and the
    geopotentials set the score while humidity contributes nothing.

    Needed for per-channel variance inflation: a single global lambda is really
    the optimum for the dominant channels, not for the ensemble.
    """
    p, t = _as_nchw(members[0]), _as_nchw(truth)
    out = []
    for c in range(p.shape[1]):
        out.append(crps_ensemble([_as_nchw(m)[:, c] for m in members], t[:, c]))
    return out
