...
def reconstruct_full_tiled_diffusion(diffusion, model, lf_full, coarse_full, ratio,
                                     ...
                                     post_x0=None, covariance_projector=None,
                                     ddim_stride=1):
    ...
    if covariance_projector is not None:
        ...
        if tuple(covariance_projector.image_size) != (tile, tile):
            raise ValueError(
                ...
                f"{(tile, tile)}: estimate/resample it at the tile size")
    ...
    def fn(origins):
        ...
        return diffusion.guided_reconstruct(
            ...
            stride=ddim_stride,
            post_x0=post_x0, covariance_projector=covariance_projector)
...
@torch.no_grad()
def reconstruct_full_tiled_residual(diffusion, model, mean_full, n_steps=100,
                                    tile=128, overlap=32, batch=8, geo_full=None,
                                    align=1, generator=None):
    """Tiled residual sampling, returning the RESIDUAL field (not composed).

    The residual model is trained on 128px patches, so one unconditional pass over
    a 480x1440 field is far off its training distribution. Measured 2026-09-06:
    full-field sampling cost the residual arm 25-53 skill points against its own
    frozen mean, while the transport arm -- same mean, same res_std, but already
    tiled -- was neutral. Every tile starts from a crop of one global noise field
    so overlapping tiles agree where the cosine window blends them."""
    noise_full = _global_noise(mean_full.shape, mean_full, generator)

    def fn(origins):
        m = crop_tiles(mean_full, origins, tile)
        z = crop_tiles(noise_full, origins, tile)
        coords = crop_geo_tiles(geo_full, origins, tile)
        return diffusion.sample_unconditional(
            model, m.shape, m.device, n_steps=n_steps, cond=(m, coords), noise=z)

    return stitch_tiles(fn, mean_full, tile, overlap, align=align, batch=batch)
